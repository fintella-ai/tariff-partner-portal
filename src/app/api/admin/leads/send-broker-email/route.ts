...
export async function POST(req: NextRequest) {
  ...
  if (lead.unsubscribedAt) {
    return NextResponse.json({ error: "This lead has unsubscribed" }, { status: 400 });
  }
...
function buildWrappedHtml(heading: string, body: string, ctaLabel: string | null, ctaUrl: string | null, unsubUrl: string): string {
...
<p style="font-size:11px;color:#999">You received this because you are listed on the CBP Permitted Customs Brokers directory. <a href="${unsubUrl}" style="color:#999;text-decoration:underline;">Unsubscribe</a></p>
</div>`;
}
