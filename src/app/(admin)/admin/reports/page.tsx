    ...
    <div>
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <p className="font-body text-[13px] text-white/40">Overview of pipeline, commissions, and partner performance.</p>
        </div>
        <div className="flex gap-2">
          <select
            className="bg-white/5 border border-white/[0.12] rounded-lg px-3 py-2 text-white font-body text-[12px] outline-none"
            value={period}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Pipeline", value: fmt$(s.totalPipeline), color: "text-white" },
          ...
          <div key={m.label} className="stat-card">
            <div className="font-body text-[9px] tracking-[1.5px] uppercase text-white/40 mb-2">{m.label}</div>
      ...
      <div className="card mb-6">
        ...
        <div className="grid grid-cols-[1fr_0.6fr_0.6fr_0.8fr_0.8fr_0.6fr] gap-4 px-6 py-3 border-b border-white/[0.06]">
          {["Month", "New Deals", "Closed Won", "Comm. Paid", "Comm. Due", "New Partners"].map((h) => (
            <div key={h} className="font-body text-[10px] tracking-[1px] uppercase text-white/35">{h}</div>
          ))}
        </div>
        {MONTHLY_DATA.map((row) => (
          <div key={row.month} className="grid grid-cols-[1fr_0.6fr_0.6fr_0.8fr_0.8fr_0.6fr] gap-4 px-6 py-3.5 border-b border-white/[0.04] last:border-b-0 items-center hover:bg-white/[0.02] transition-colors">
            <div className="font-body text-[13px] text-white/80">{row.month}</div>
            <div className="font-body text-[13px] text-white/60">{row.newDeals}</div>
            <div className="font-body text-[13px] text-white/60">{row.closedWon}</div>
      ...
      <div className="card">
        ...
        <div className="grid grid-cols-[0.3fr_1.5fr_0.6fr_0.6fr_0.8fr_0.8fr] gap-4 px-6 py-3 border-b border-white/[0.06]">
          {["#", "Partner", "Code", "Deals", "Pipeline", "Commission"].map((h) => (
            <div key={h} className="font-body text-[10px] tracking-[1px] uppercase text-white/35">{h}</div>
          ))}
        </div>
        {TOP_PARTNERS.map((p, i) => (
          <div key={p.code} className="grid grid-cols-[0.3fr_1.5fr_0.6fr_0.6fr_0.8fr_0.8fr] gap-4 px-6 py-3.5 border-b border-white/[0.04] last:border-b-0 items-center hover:bg-white/[0.02] transition-colors">
            <div className={`font-display text-sm font-bold ${i === 0 ? "text-yellow-400" : i === 1 ? "text-gray-300" : i === 2 ? "text-orange-400" : "text-white/30"}`}>
              {i + 1}
            </div>
            <div className="font-body text-[13px] text-white/80">{p.name}</div>
            <div className="font-body text-[11px] text-white/30 tracking-wider">{p.code}</div>
            <div className="font-body text-[13px] text-white/60">{p.deals}</div>
            <div className="font-body text-[13px] text-white/60">{fmt$(p.pipeline)}</div>
